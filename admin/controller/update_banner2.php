    ...
    if(isset($_POST['add_banner2'])){
        ...
        if($banner_size <= 500000){
            if(in_array($banner_ext, $allowed_ext)){
                if(move_uploaded_file($_FILES['banner2']['tmp_name'], $banner_folder)){
                    $add_photo = $connectdb->prepare("UPDATE exhibitors SET banner2 = :banner2 WHERE exhibitor_id = :exhibitor_id");
                    // $add_photo->bindvalue('banner_description', $description);
                    $add_photo->bindvalue('banner2', $banner);
